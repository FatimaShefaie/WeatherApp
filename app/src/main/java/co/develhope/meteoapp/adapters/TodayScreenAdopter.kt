        ...
        class TodayCardViewHolder(private val todayCard : TodayCardViewBinding):
                RecyclerView.ViewHolder(todayCard.root){
                    fun bind( title: TodayDataClass.TodayCardViewClass){
                        todayCard.tvTdPrecipitationGrade.text = itemView.context.getString(R.string.td_precipitation_grade,title.feelsLike)
                        todayCard.tdUvIndex.text = itemView.context.getString(R.string.td_index, title.uvIndex)
                        todayCard.tdHumidityGrade.text = itemView.context.getString(R.string.td_hum_grade, title.humidity)
                        todayCard.tdWindSpeed.text = itemView.context.getString(R.string.td_hum_grade, title.windSpeed)
                        todayCard.tdCoveragePer.text = itemView.context.getString(R.string.coverage_percent_td, title.cloudCover)
                        todayCard.tdRainPer.text = itemView.context.getString(R.string.rain_mm_td, title.rainAmount)
        ...
        class TodayHourlyViewHolder(private val todayHourly : TodayHourlyItemBinding):
                RecyclerView.ViewHolder(todayHourly.root){
                    fun bind(title: TodayDataClass.TodayHourlyClass){
                        todayHourly.td2Hour.text = itemView.context.getString(R.string.td2_hour, title.date.hour)
                        todayHourly.td2Grade.text = itemView.context.getString(R.string.td2_grade, title.temp)
                        todayHourly.td2HumidityPercent.text = itemView.context.getString(R.string.td2_humidity_percent, title.pericip)
                    }
                }
        ...
        return when (viewType)
        {
            ...
            HOURLY ->{
                TodayHourlyViewHolder(
                    TodayHourlyItemBinding.inflate(LayoutInflater.from(parent.context),parent,false))
            }
